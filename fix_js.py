...
old_js = """async function renderPrintArea(inv) {
...
new_js = r"""async function renderPrintArea(inv) {
  const shop = inv.shopSnapshot || await DB.getSettings();
  const el = document.getElementById('print-content');
...
old_js_regex = re.escape(old_js).replace(r'\n', r'\s*')

match = re.search(old_js_regex, text)
if match:
    text = text[:match.start()] + new_js + text[match.end():]
    with open('index.html', 'w', encoding='utf-8') as f:
        f.write(text)
    print("Successfully replaced JS!")
else:
    print("Could not find old JS with regex.")
